...
const INTERACTIVE_EFFECTS = new Set(["investigate", "forceAttack", "replayTopGraveAbility", "search", "replayAbility", "additionalTargetCost", "optionalRedirect", "optionalDrawFrom", "removeMarkersFromConstants"]);
  ...
  while (stack.length) {
    ...
    if (item.kind === "command") {
      if (item.command.type === "passPriority") {
        ...
        if (!attacker || attacker.exhausted || attacker.attackedThisTurn || attacker.summoning || attacker.stunned || hasKeyword(attacker, /atordoado/i)) throw new RulesViolation("invalid-attacker"); if (!attackPermissionMet(attacker)) throw new RulesViolation("attack-requirement-not-met");
