...
export function executeCommand(inputState, command, options = {}) {
    ...
    if (item.kind === "command") {
      if (item.command.type === "playCard") {
        ...
        if (permanent) {
          ...
          else { if (entry.support.length >= 5 || entry.support.some((existing) => existing.slot === unit.slot)) throw new RulesViolation("support-zone-full"); if (card.type === "Artefato") { const attached = entry.board.find((creature) => creature.uid === item.command.attachedTo); if (!attached || entry.support.some((artifact) => artifact.attachedTo === attached.uid)) throw new RulesViolation("artifact-target-required"); unit.attachedTo = attached.uid; unit.slot = attached.slot; } entry.support.push(unit); }
  ...
  if (["playCard", "activate"].includes(command.type)) state.pendingResponse = command.hasPriority ? null : { responder: 1 - command.owner, actor: command.owner, action: actionLabel };
