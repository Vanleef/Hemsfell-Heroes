...
import { nextTurnPhase } from "./state/match-state.mjs";
...
function activationTargetSteps(ability, sourceId = null) {
  const costSteps = (ability.costs || []).flatMap((cost) => cost.type === "sacrifice"
    ? Array.from({ length: Number(cost.amount) || 1 }, () => ({ scope: TargetScope.ALLY_CREATURE, role: "sacrifice", requiredSubtype: cost.subtype }))
    : []);
  return [...costSteps, ...abilityTargetSteps(ability, sourceId)];
}
...
function activeAbilities(state, event) {
  ...
  state.players.forEach((entry, owner) => {
    ...
    if (entry.heroId === "ngoro" && event.owner === owner && (event.type === "onMaintenanceResourceChoice" || (event.type === "onMaintenance" && event.afterResourceChoice === true))) result.push({ source: heroSource, owner, ability: { id: "ngoro-level-1-maintenance", trigger: "onMaintenanceResourceChoice", effects: [{ type: "chooseDeckAndInvestigate", amount: 1 }] } });
  ...
  while (stack.length) {
    ...
    if (item.kind === "command") {
      if (item.command.type === "evolveHero") {
        ...
        const activationSteps = activationTargetSteps(ability);
        if (activationSteps.length && !((item.command.targetIds || []).length + (item.command.sacrificeIds || []).length)) { if (!canSatisfyTargetSteps(state, item.command.owner, activationSteps)) throw new RulesViolation("ability-not-available"); state.pendingDecision = { kind: "activation-targets", owner: item.command.owner, effect: {}, context: { owner: item.command.owner, sourceId: source.uid }, targetSteps: activationSteps, sourceName: source.name, command: { ...item.command } }; continue; }
        ...
        if (decision.kind === "search") {
          ...
          if (effect.destination === "hand") entry.hand.push(...selected.map((card) => effect.reveal || effect.name || effect.nameIncludes ? { ...card, revealed: true, revealedTo: [0, 1] } : card));
            ...
            const { revealed: _revealed, revealedTo: _revealedTo, ...fieldCard } = card;
            const unit = { ...fieldCard, uid: `${card.id}-${state.round}-searched`, enteredRound: state.round, attackedThisTurn: false, damage: 0, exhausted: false, summoning: card.type === "Criatura", modifiers: [], abilities: card.abilities || [] };
        ...
        if (decision.kind === "targets" || decision.kind === "activation-targets") {
          ...
          if (decision.kind === "activation-targets") { const sacrificeIds = targetIds.filter((_, index) => steps[index]?.role === "sacrifice"), effectTargetIds = targetIds.filter((_, index) => steps[index]?.role !== "sacrifice"); state.pendingDecision = null; stack.push(...continuation); stack.push({ kind: "command", command: { ...decision.command, sacrificeIds, targetIds: effectTargetIds } }); continue; }
        }
