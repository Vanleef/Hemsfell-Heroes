...
export function chooseAIHeroAbility(state, owner, difficulty = "Normal") {
  ...
  if (entry.heroId === "ngoro" && entry.level >= 3 && !used(2) && Number(entry.heroXP || 0) >= 3) { const target = (entry.board || []).sort((a, b) => Number(b.atk || 0) - Number(a.atk || 0))[0]; if (target) return { kind: "ngoro-stealth", slot: 2, targetId: cardId(target) }; }
