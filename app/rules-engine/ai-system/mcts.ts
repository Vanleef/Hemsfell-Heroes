...
class Node {
  ...
  constructor(parent: Node | null, action: AIAction | null, actions: AIAction[], actor: number, priorBias = 0) {
    this.parent = parent;
    this.action = action;
...
export class MCTS {
  async search(options: MCTSOptions): Promise<{ action: AIAction | null; stats: SearchStats }> {
    ...
    const risk = options.riskManager || new RiskManager();
    ...
    if (!rootActions.length) return { action: null, stats: { iterations: 0, elapsedMs: 0, rootVisits: 0, selectedVisits: 0, selectedMeanValue: 0, iterationsPerSecond: 0, beliefEntropy: belief.entropy() } };

    while (iterations < config.iterations && now() - started < config.thinkTimeMs) {
      let simulation = belief.determinize(state, owner);
      let node = root;

      // Selection is adversarial. A small decaying prior from RiskManager lets
      // hold-response/overextension knowledge influence UCT without becoming a
      // second post-search decision system.
      ...
      // Expansion computes a strategic prior using the same risk policy used in
      // rollouts. This makes MCTS naturally compare "develop" vs "hold/pass".
      while (node.untried.length && simulation.winner == null) {
        ...
        const next = this.safeApply(adapter, before, action);
        if (!next) continue;
        const actor = actorForState(before, owner);
        const signedBias = risk.actionBias(before, next, owner, personality, action, random) * (actor === owner ? 1 : -1);
        simulation = next;
        ...
        const child = new Node(node, action, actions, nextActor, signedBias);
        node.children.push(child);
        node = child;
        break;
      }

      simulation = this.rollout(simulation, owner, config, personality, adapter, evaluator, risk, legacyDifficulty, random);
    ...
    return {
      ...
      stats: {
        ...
        iterationsPerSecond: elapsedMs > 0 ? iterations * 1000 / elapsedMs : 0,
  ...
  private selectUCT(node: Node, rootOwner: number, exploration: number, random: () => number): Node {
    ...
    for (const child of node.children) {
      ...
      const riskPrior = child.priorBias / Math.sqrt(1 + child.visits);
      const score = exploitation + explore + riskPrior + random() * 1e-9;
      if (score > bestScore) { best = child; bestScore = score; }
    }
    return best;
  }

  private rollout(state: AIGameState, rootOwner: number, config: DifficultyConfig, personality: PersonalityProfile, adapter: EngineAdapter, evaluator: Evaluator, risk: RiskManager, difficulty: string, random: () => number): AIGameState {
    let current = state;
    for (let depth = 0; depth < config.rolloutDepth && current.winner == null; depth += 1) {
      ...
      if (random() < config.heuristicRolloutChance) {
        const scored = actions.map((candidate) => {
          ...
          const actionBias = risk.actionBias(current, next, rootOwner, personality, candidate, random);
          const signedUtility = actor === rootOwner ? ownerUtility + actionBias : -ownerUtility - actionBias;
          return { candidate, next, score: signedUtility };
