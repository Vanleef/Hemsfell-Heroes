...
export function roomView(room: Room, includeGame = false, role?: RoomRole | null) {
  return { id: room.id, host: { heroId: room.host.heroId, accepted: room.host.accepted, deckLocked: room.host.deckLocked, mulliganDone: room.host.mulliganDone, mulliganCount: room.host.mulliganCount, mulliganDeadline: room.host.mulliganDeadline ?? null, disconnectedAt: room.host.disconnectedAt ?? null, rematchRequested: !!room.host.rematchRequested }, guest: room.guest ? { heroId: room.guest.heroId, accepted: room.guest.accepted, deckLocked: room.guest.deckLocked, mulliganDone: room.guest.mulliganDone, mulliganCount: room.guest.mulliganCount, mulliganDeadline: room.guest.mulliganDeadline ?? null, disconnectedAt: room.guest.disconnectedAt ?? null, rematchRequested: !!room.guest.rematchRequested } : null, status: room.status, settings: room.settings, coinWinner: room.coinWinner, startingRole: room.startingRole, createdAt: room.createdAt, revision: room.revision, ...(includeGame ? { game: role ? publicGameView(room, role) : null } : {}) };
}
