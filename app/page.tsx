...
const activeCardEffect=(card:CardDef,player:Player,owner:0|1,response:PendingResponse|null)=>{const cost=effectiveCost(card,player),lifeLoss=Number(card.text.match(/\bperca\s+(\d+)\s+(?:de\s+)?vida/i)?.[1]||0),needsSacrifice=/sacrifique[^.]*criatura/i.test(card.text);if(cost>playableEnergy(card,player)||(lifeLoss&&player.life<=lifeLoss)||(needsSacrifice&&!player.board.length))return"";if(card.tags.some(tag=>cleanName(tag)==="fura fila")&&player.turnCardsPlayed>0)return"FURA-FILA";const element=cardElement(card);if(element&&player.elementChain?.element===element)return`CADEIA · ${player.elementChain.effect.toUpperCase()}`;if(needsSacrifice)return"SACRIFÍCIO";if(lifeLoss)return`PERDA · ${lifeLoss}`;if(isFast(card)&&response?.responder===owner)return"RESPOSTA";if(cost!==card.cost)return cost<card.cost?"CUSTO ↓":"CUSTO ↑";return""};
 const hasActivatableUnitEffect=(unit:Unit)=>hasActivatableEffect(unit);
...
function OriginalCard({card,controller,small=false,disabled=false,selected=false,targetClass="",activeEffect="",priority=false,draggable=false,onDragStart,onDragEnd,onClick,onActivate,activationDisabled=false}:{card:CardDef|Unit;controller?:Player;small?:boolean;disabled?:boolean;selected?:boolean;targetClass?:string;activeEffect?:string;priority?:boolean;draggable?:boolean;onDragStart?:(e:React.DragEvent)=>void;onDragEnd?:()=>void;onClick?:()=>void;onActivate?:()=>void;activationDisabled?:boolean}){
 ...
 <RemoteCardArt page={card.page} name={card.name} priority={priority}/>{costChanged&&<span className={`effective-cost ${shownCost<card.cost?"discounted":"modified"}`} title={`Custo base ${card.cost}; custo atual ${shownCost}`}>{shownCost}</span>}{card.imageCard&&<span className="image-badge">IMAGEM</span>}{card.revealed&&<span className="revealed-badge">REVELADA</span>}{unit&&<><span className={`live-atk ${modifiers.atk>0?"is-buffed":modifiers.atk<0||unit.frozen?"is-weakened":""}`}>{liveAttack}</span><span className={`live-hp ${modifiers.hp>0?"is-buffed":modifiers.hp<0?"is-weakened":""}`}>{liveVitality}</span>{activeEffect&&<i className="active-effect-label">{activeEffect}</i>}{(unit.exhausted||unit.frozen||unit.stunned||unit.suffocated||unit.immobilized)&&<i className="status">{unit.suffocated?"SUFOCADA":unit.stunned?"ATORDOADA":unit.frozen?"CONGELADA":unit.immobilized?"IMOBILIZADA":"VIRADA"}</i>}</>}
...
useEffect(()=>{if(mode!=="online"||!roomId||!roomToken||!game)return;const deadline=game.pendingResponse?.deadline??game.turnDeadline;if(!deadline||deadline>clockNow)return;const key=`${game.round}-${game.pendingResponse?.action??"turn"}-${deadline}`;if(timeoutSentRef.current===key)return;timeoutSentRef.current=key;roomAction("timeout")},[clockNow,mode,roomId,roomToken,game?.pendingResponse?.deadline,game?.turnDeadline]);
 ...
 return <main className={`hh-app screen-${screen}`}>
  ...
  {screen==="game"&&game&&me&&foe&&<section className="hs-board">
   ...
   <div className="player-hand">{me.hand.map((c,i)=><OriginalCard key={c.id} card={c} controller={me} priority activeEffect={activeCardEffect(c,me,0,responseWindow)} disabled={priorityLocked||game.active!==0||game.phase!=="principal"||effectiveCost(c,me)>playableEnergy(c,me)} draggable onDragStart={e=>{setDragging({index:i,type:c.type});e.dataTransfer.setData("card-index",String(i));e.dataTransfer.setData("text/plain",String(i));e.dataTransfer.effectAllowed="move"}} onDragEnd={()=>setDragging(null)} onClick={()=>setShowInspector(c)}/>)}</div>
...
function TerrainSlot({card,enemy=false,drop=false,dragIndex,onDrop,targetClass="",onTarget}:{card:Unit|null;enemy?:boolean;drop?:boolean;dragIndex?:number;onDrop?:(i:number)=>void;targetClass?:string;onTarget?:()=>void}){
 return <div className={`terrain-slot ${enemy?"enemy-terrain":"player-terrain"} ${drop?"can-drop":""}`} onDragOver={e=>{if(drop){e.preventDefault();e.dataTransfer.dropEffect="move"}}} onDrop={e=>{e.preventDefault();if(drop)onDrop?.(dragIndex??Number(e.dataTransfer.getData("card-index")||e.dataTransfer.getData("text/plain")))}}>
  <em>TERRENO CRUEL · 1</em>{card?<OriginalCard card={card} small priority targetClass={targetClass} onClick={onTarget}/>:<span>◇</span>}
 </div>
}
