...
const poolFor=(id:string)=>{const supplied=suppliedDeckPages[id as DeckId];return supplied?supplied.map(([page])=>cardsByPage.get(page)).filter((card):card is CardDef=>!!card&&!card.imageCard&&!disabledDeckCardIds.has(card.id)):allFor(id).filter(c=>!c.imageCard&&!disabledDeckCardIds.has(c.id))};
const collectionFor=(id:string)=>{const supplied=suppliedDeckPages[id as DeckId];if(supplied)return supplied.map(([page,quantity])=>({card:cardsByPage.get(page),quantity})).filter((entry):entry is {card:CardDef;quantity:number}=>!!entry.card&&!entry.card.imageCard&&!disabledDeckCardIds.has(entry.card.id));const pool=poolFor(id),base=Math.floor(49/Math.max(1,pool.length)),remainder=49%Math.max(1,pool.length);return pool.map((card,index)=>({card,quantity:base+(index<remainder?1:0)}))};
...
const baseCard=(c:CardDef|Unit):CardDef=>{const template=cardsByPage.get(c.page)??c;return{page:template.page,id:c.id,name:template.name,type:template.type,cost:template.cost,atk:template.atk,hp:template.hp,text:template.text,tags:[...template.tags],subtypes:[...(template.subtypes||[])],abilities:template.abilities?structuredClone(template.abilities):undefined,rules:template.rules,diagnostics:template.diagnostics,image:template.image,hero:template.hero,imageCard:template.imageCard,generatedImage:c.generatedImage}};
...
useEffect(()=>{const inspect=(event:Event)=>{const detail=(event as CustomEvent<Partial<CardDef>>).detail,page=Number(detail?.page),printed=cardsByPage.get(page);if(printed)setShowInspector(printed)};window.addEventListener("hemsfell:inspect-card",inspect);return()=>window.removeEventListener("hemsfell:inspect-card",inspect)},[]);
// Online room state
...
useEffect(()=>{if(mode!=="online"||roomInfo?.status!=="mulligan")return;const participant=isHost?roomInfo?.host:roomInfo?.guest,deadline=participant?.mulliganDeadline;if(participant?.mulliganDone||!deadline)return;const key=`mulligan-${deadline}`;const expire=()=>{if(timeoutSentRef.current===key)return;timeoutSentRef.current=key;void roomAction("timeout")};const delay=deadline-Date.now();if(delay<=0){expire();return}const timer=window.setTimeout(expire,delay+25);return()=>window.clearTimeout(timer)},[mode,isHost,roomInfo?.status,roomInfo?.host?.mulliganDone,roomInfo?.host?.mulliganDeadline,roomInfo?.guest?.mulliganDone,roomInfo?.guest?.mulliganDeadline]);
 ...
 const resolveUrukLevelOne=(g:Game,owner:0|1,targetUid?:string)=>{
  ...
  const heroCard=cardsByPage.get(deckById(p.heroId).heroPage),spellName=p.lastElementSource||"último feitiço";
 ...
 const responseSourceId=typeof sourceCommand?.sourceId==="string"?sourceCommand.sourceId:typeof sourceCommand?.attackerId==="string"?sourceCommand.attackerId:typeof sourceCommand?.cardId==="string"?sourceCommand.cardId:sourceUnits.length===1?sourceUnits[0].uid:undefined;
 const priorityStackCard=typeof sourceCommand?.cardId==="string"?cardsById.get(sourceCommand.cardId):undefined;
 const priorityAttacker=typeof sourceCommand?.attackerId==="string"?game?.players.flatMap(player=>player.board).find(unit=>unit.uid===sourceCommand.attackerId):undefined;
 const prioritySourceLabel=priorityAttacker?`Atacante · ${priorityAttacker.name}`:priorityStackCard?`Carta na pilha · ${priorityStackCard.name}`:sourceUnits.length===1?`Origem · ${sourceUnits[0].name}`:undefined;
 ...
 const selectedPool=useMemo<CardDef[]>(()=>activeUserDeck.main.flatMap(entry=>{const card=cardsById.get(entry.cardId);return card?[{...card,collectionQuantity:entry.quantity}]:[]}),[activeUserDeck]);
 const selectedExtra=useMemo(()=>activeUserDeck.extra.map(cardId=>cardsById.get(cardId)).filter((card):card is CardDef=>!!card),[activeUserDeck]);
 ...
 return <main className={`hh-app screen-${screen}`}>
  ...
  {screen==="decks"&&<section className="collection">
   ...
   <div className="deck-detail"><aside style={{"--deck":selectedDeck.color} as React.CSSProperties}>
    <button className="collection-hero-inspect" onClick={()=>setShowInspector(cardsByPage.get(selectedDeck.heroPage)||null)} aria-label={`Ver detalhes de ${selectedDeck.name}`}><RemoteCardArt page={selectedDeck.heroPage} name={selectedDeck.name} priority/></button>
  ...
  {screen==="game"&&game&&me&&foe&&<div className="game-stage"><section className="hs-board game-content" data-card-dragging={dragging?"true":undefined}>
   ...
   <div className="hero-panel-stack canonical-hero-panel enemy"><PlayerHero player={foe} enemy targetClass={engineTargetIds.includes("enemy-hero")?"target-enemy":enemyHeroTarget?"target-enemy":""} onTarget={engineTargetIds.includes("enemy-hero")?()=>selectEngineTarget("enemy-hero"):enemyHeroTarget?()=>applyTarget("enemy-hero"):undefined} onInspect={()=>setShowInspector(cardsByPage.get(deckById(foe.heroId).heroPage)||null)}/><HeroAbilities player={foe} enemy/></div><div className="opponent-hand" style={{"--hand-count":Math.max(1,foe.hand.length)} as React.CSSProperties}>{foe.hand.map((card,i)=>card.revealed?<OriginalCard key={card.id} card={card} small onClick={()=>setShowInspector(card)}/>:<button type="button" className="opponent-card-back official-card-back" data-card-id={card.id} key={card.id||i} aria-label="Carta oculta do oponente"/>)}</div>
