...
/**
 * Convert the server's canonical host-oriented game snapshot into the guest's
 * local perspective. Card ownership arrays are swapped and every player index
 * carried by the Online timing model is flipped exactly once.
 *
 * This helper is deliberately pure so reconnect/polling paths can share the
 * same transformation without mutating the authoritative room snapshot.
 */
export function orientOnlineGameForRole(input, role = "host") {
  ...
  if (game.pendingResponse) {
    game.pendingResponse = {
      ...game.pendingResponse,
      responder: flip(game.pendingResponse.responder),
      actor: flip(game.pendingResponse.actor),
    };
  }

  if (game.pendingAction) game.pendingAction = orientCommand(game.pendingAction);

  if (Array.isArray(game.priorityStack)) {
    game.priorityStack = game.priorityStack.map((frame) => ({
      ...frame,
      actor: flip(frame.actor),
      command: orientCommand(frame.command),
    }));
  }
  ...
  if (Array.isArray(game.stack)) {
    game.stack = game.stack.map((frame) => ({
      ...frame,
      controller: flip(frame.controller),
      command: orientCommand(frame.command),
    }));
  }
