...
export const activatedAbilities = (cardOrText = "") => {
  ...
  return compiled.abilities.filter((ability) =>
    ability.trigger === "activated" && (explicit || ability.uiActivation || ability.costs.some(isExplicitBoardCost)),
  );
};
...
export function canActivateCard(card, context) {
  ...
  const ability = activatedAbilities(card)[0];
  if (!ability) return false;
  if (ability.availability?.topGraveHasTrigger) {
    const top = context.topGrave;
    if (!top || top.type !== "Criatura" || !compileCard(top).abilities.some((candidate) => candidate.trigger === ability.availability.topGraveHasTrigger)) return false;
  }
