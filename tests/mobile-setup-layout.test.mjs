...
const css = await readFile(
  new URL("../app/presentation/styles/match-ui-guard.css", import.meta.url),
  "utf8",
);
...
test("mobile deck setup remains contained inside the viewport", () => {
  ...
  assert.match(css, /\.deck-picker > select\s*\{[\s\S]*?min-width:\s*0[\s\S]*?text-overflow:\s*ellipsis/);
});
