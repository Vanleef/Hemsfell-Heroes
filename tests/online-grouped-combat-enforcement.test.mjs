...
const passCombatResolution = (game) => {
...
test("one attacker immediately owns one defender decision and blocks further attacker actions", () => {
  const game = executeOnlineCommand(combatState(), { type: "declareAttack", owner: 0, attackerId: "attacker" }).state;
...
test("blocker selection is unitary and server resolves after the post-block response window", () => {
  ...
  game = passCombatResolution(game);
...
test("illegal blocker is rejected before a block is committed", () => {
