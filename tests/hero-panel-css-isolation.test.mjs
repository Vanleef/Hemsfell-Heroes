...
test("canonical ability layout uses the full board path to outrank legacy important rules", () => {
  assert.match(heroCss, /html body \.screen-game\.screen-game \.game-stage > \.game-content\.hs-board > \.hero-panel-stack\.canonical-hero-panel > \.hero-command-bar\s*\{[\s\S]*?grid-template-rows:\s*repeat\(3, minmax\(0, 1fr\)\)/);
  assert.match(heroCss, /html body \.screen-game\.screen-game \.game-stage > \.game-content\.hs-board > \.hero-panel-stack\.canonical-hero-panel > \.hero-command-bar > \.hero-ability-chip\s*\{[\s\S]*?display:\s*grid\s*!important/);
  assert.match(heroCss, /html body \.screen-game\.screen-game \.game-stage > \.game-content\.hs-board > \.hero-panel-stack\.canonical-hero-panel > \.hero-command-bar > \.hero-ability-chip > \.hero-ability-copy\s*\{[\s\S]*?grid-template-columns:\s*max-content minmax\(0, 1fr\)/);
});
