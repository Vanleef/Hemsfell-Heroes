...
test("detailed copy preserves important limits, costs and durations", () => {
  assert.match(runtime, /Tifon[\s\S]*?A primeira vez em cada um dos seus turnos/);
  assert.match(runtime, /Saymon[\s\S]*?pague 2 de vida[\s\S]*?Roubo de Vida permanentemente/);
  assert.match(runtime, /Ngoro[\s\S]*?Remova 2 Pistas[\s\S]*?Remova 3 Pistas/);
  assert.match(runtime, /natureza:[\s\S]*?Uma vez por turno[\s\S]*?Remova um total de 4 marcadores/);
});
