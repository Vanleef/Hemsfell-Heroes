...

{
  ...
  // Field cards already communicate their active state through glow, keyword icons and
  // activation affordances. Remove the extra text plaque so the board stays readable.
  source = replaceOnce(
    source,
    ...
    "remove active effect plaque"
  );

  // Tessalia's center creature is mechanically the Commander. Give that permanent a
  // dedicated semantic class so its identity is visible without adding another label.
  source = replaceOnce(
    source,
    ...
    "tessalia commander visual flag"
  );
  source = replaceOnce(
    source,
    'return <span className={`card-frame ${small?"is-small":""}`} data-unit-id={unit?.uid}>',
    'return <span className={`card-frame ${small?"is-small":""} ${tessaliaCommander?"tessalia-commander-frame":""}`} data-unit-id={unit?.uid}>',
    "tessalia commander frame class"
  );
