...
const helper = `function passExpiredResponseWindow() {
...
if (!guard.includes("function passExpiredResponseWindow()")) {
  if (!guard.includes(marker)) throw new Error("Could not locate MatchUiGuard export marker");
  guard = guard.replace(marker, helper + marker);
}

const oldBlock = `    const observer = new MutationObserver(scheduleSync);\n    observer.observe(document.body, { childList: true, subtree: true });\n    sync();\n    return () => {\n`;
const newBlock = `    const observer = new MutationObserver(scheduleSync);\n    observer.observe(document.body, { childList: true, subtree: true });\n    const responseTimeoutTimer = window.setInterval(passExpiredResponseWindow, 200);\n    sync();\n    return () => {\n      window.clearInterval(responseTimeoutTimer);\n`;
if (!guard.includes("responseTimeoutTimer")) {
  if (!guard.includes(oldBlock)) throw new Error("Could not locate MatchUiGuard observer cleanup block");
  guard = guard.replace(oldBlock, newBlock);
}
fs.writeFileSync(guardPath, guard);

const testPath = "tests/cafe-pingado-priority-regression.test.mjs";
